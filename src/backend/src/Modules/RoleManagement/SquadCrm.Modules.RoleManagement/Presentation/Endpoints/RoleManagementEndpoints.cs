...
namespace SquadCrm.Modules.RoleManagement.Presentation.Endpoints;

/// <summary>
/// HTTP surface of the RoleManagement module: route definitions, model binding,
/// status mapping and response projection. Composed by
/// <see cref="RoleManagementModule"/>, which owns registration only.
/// </summary>
internal static class RoleManagementEndpoints
{
    ...
    {
        Role? role = await roleService.GetAsync(id, cancellationToken);
    ...
    private static async Task<IResult> ListAsync(
        ...
        CancellationToken cancellationToken)
    {
        PagedResult<Role> page = await roleService.ListAsync(pagination, cancellationToken);
