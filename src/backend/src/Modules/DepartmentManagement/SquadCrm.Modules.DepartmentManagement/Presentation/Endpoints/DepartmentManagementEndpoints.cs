...
using SquadCrm.Modules.DepartmentManagement.Presentation.Responses;

namespace SquadCrm.Modules.DepartmentManagement.Presentation.Endpoints;

/// <summary>
/// HTTP surface of the DepartmentManagement module: route definitions, model binding,
/// status mapping and response projection. Composed by
/// <see cref="DepartmentManagementModule"/>, which owns registration only.
/// </summary>
internal static class DepartmentManagementEndpoints
{
    ...
    {
        Department? department = await departmentService.GetAsync(id, cancellationToken);
    ...
    private static async Task<IResult> ListAsync(
        ...
        CancellationToken cancellationToken)
    {
        PagedResult<Department> page = await departmentService.ListAsync(pagination, cancellationToken);
